...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    if (savedVoucher) {
      ...
      // Only apply and clear if we have a valid total calculated
      if (grossTotal > 0) {
  ...
  const handleApplyVoucher = async (codeToApply) => {
    ...
    try {
      setError(null);
      // Backend expects 'code' and 'orderTotal' (standard camelCase)
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const redirectUrl = orderData?.paymentUrl || orderData?.paymentLinkUrl;
        if (redirectUrl) {
          // Check if it's a mock URL or real PayMongo URL
          if (redirectUrl.startsWith('/mock-checkout')) {
  ...
  // Verify payment status on return from PayMongo
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (!orderId || !params.get('payment_return')) return;

    const verifyPayment = async () => {
      try {
        setIsProcessing(true); // Reuse processing state to show loading/disable buttons
        const res = await ordersApi.verify(orderId);
        ...
        // Don't show error immediately so user can retry or check status manually
      } finally {
        setIsProcessing(false);
      }
    };
