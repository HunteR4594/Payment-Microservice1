...
const RefundAdminPage = () => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      alert(`Refund approved. Please click "Process Refund" to credit ₱${selectedRefund.amount} to the wallet.`);
      await loadRefunds();
          ...
          <tbody>
            {refunds.map((refund) => (
              <tr key={refund.id}>
                ...
                <td>
                  {refund.status?.toLowerCase() === 'completed' || refund.walletCredited ? (
                    <span className="completed-text">✓ Completed</span>
                  ) : canProcessToWallet(refund.status) ? (
                    <button
                      className="process-btn"
                      onClick={() => handleProcessToWallet(refund.id)}
                      disabled={processing}
                    >
                      Process Refund
                    </button>
            ...
            <div className="modal-body">
              <div className="detail-grid">
                ...
                <div className="detail-item">
                  ...
                  <span style={{ fontWeight: 'bold', color: 'var(--primary-color)' }}>₱{selectedRefund.amount}</span>
                </div>
                {selectedRefund.voucherCode && (
                  <>
                    <div className="detail-item">
                      <label>Voucher Used</label>
                      <span className="badge-info" style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '0.85em' }}>{selectedRefund.voucherCode}</span>
                    </div>
                    <div className="detail-item">
                      <label>Voucher Discount</label>
                      <span style={{ color: '#28a745' }}>-₱{selectedRefund.voucherDiscount}</span>
                    </div>
                  </>
                )}
              </div>
