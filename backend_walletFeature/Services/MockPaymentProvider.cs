...
public class MockPaymentProvider : IPaymentProvider
{
    ...
    public Task<CreatePaymentLinkResponse> CreatePaymentLinkAsync(decimal amount, string description, string referenceId)
    {
        var mockLink = new PaymentLink
        {
            Id = $"mock_link_{Guid.NewGuid():N}"[..16],
            // Use relative URL so it works on any frontend_walletFeature port
